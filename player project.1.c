...
void ensureCapacity() {
    if (playercount >= capacity) {
        ...
        struct player *temp = realloc(players, capacity * sizeof(struct player));
        if (temp == NULL) {
            printf("Memory allocation failed!\n");
            exit(1);
        }
        players = temp;
        printf("Capacity increased to %d players.\n", capacity);
    }
}
...
void updateplayer() {
    ...
    printf("Enter a jersey number to update (0 to cancel): ");
    scanf("%d", &jersey);
...
void displaysortedplayers() {
    ...
    struct player tempPlayers[playercount];
    for (int i = 0; i < playercount; i++) {
